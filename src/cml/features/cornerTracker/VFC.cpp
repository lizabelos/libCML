...
bool CML::Features::VFC::setData(List<Vector2f> X1, List<Vector2f> X2) {
...
CML::List<int> CML::Features::VFC::obtainCorrectMatch() {
	return _matchIdx;
}

void CML::Features::VFC::optimize() {
	if (!normalize())
		return;
...
void CML::Features::VFC::optimizeSparseVFC() {
	_numCtrlPts = std::min(_numCtrlPts, _numPt);
	selectSubset();
...
bool CML::Features::VFC::selectSubset() {
	_numCtrlPts = std::min(_numCtrlPts, _numPt);
	_ctrlPts.clear();
	int cnt = 0;
	int iter = 0;
...
void CML::Features::VFC::calculateC_SparseVFC() {
	Matrixf<Dynamic, Dynamic> K(_numCtrlPts, _numCtrlPts);
	for (int i = 0; i < _numCtrlPts; i++) {
		for (int j = i; j < _numCtrlPts; j++) {
			...
			tmp += _lambda * _sigma2 * _K(i, j);
			K(i, j) = tmp;
			K(j, i) = tmp;
		}
	}
	Matrixf<Dynamic, Dynamic> Y(_numCtrlPts, 2);
	for (int i = 0; i < _numCtrlPts; i++) {
		float tmp1 = 0;
		float tmp2 = 0;
		auto p = _U.row(i);
	...
	Matrixf<Dynamic, Dynamic> C;
	C = K.lu().solve(Y);
	_C.clear();
	for (int i = 0; i < _numCtrlPts; i++) {
		float t1 = C(i, 0);
		float t2 = C(i, 1);
		_C.push_back(Vector2f(t1, t2));
	}
}


void CML::Features::VFC::optimizeFastVFC() {
	_K = constructIntraKernel(_X);
	_numEig = static_cast<int>( sqrt(_numPt) + 0.5f );
	auto solver = Eigen::SelfAdjointEigenSolver<Matrixf<Dynamic, Dynamic>>(_K);
	_S = solver.eigenvalues();
	_Q = solver.eigenvectors();
...
void CML::Features::VFC::calculateTraceCQSQC() {
	_traceCQSQC = 0;
	for (int i = 0; i < _numEig; i++) {
		...
		_traceCQSQC += t1*t1*_S(i, 0);
		_traceCQSQC += t2*t2*_S(i, 0);
	}
}

void CML::Features::VFC::calculateCFastVFC() {
	Matrixf<Dynamic, Dynamic> dPQ(_numEig, _numPt);
	Matrixf<Dynamic, Dynamic> F(2, _numPt);
	...
	Matrixf<Dynamic, Dynamic> QF(_numEig, 2);
	for (int i = 0; i < _numEig; i++) {
		auto q = _Q.row(i);
		float t1 = 0;
		float t2 = 0;
	...
	// Q'*dPQ
	Matrixf<Dynamic, Dynamic> K(_numEig, _numEig);
	for (int i = 0; i < _numEig; i++) {
		for (int j = i; j < _numEig; j++) {
			...
			if (i != j) {
				...
				K(i, i) = tmp + _lambda * _sigma2 / _S(i, 0);
			}
		}
	}
	
	Matrixf<Dynamic, Dynamic> T;
	T = K.lu().solve(QF);
	//Matrixf<Dynamic, Dynamic> C(2, _numPt, CV_32FC1);
	Matrixf<Dynamic, Dynamic> C = F - (T.transpose() * dPQ);
	C = C / (_lambda*_sigma2);
...
void CML::Features::VFC::optimizeVFC() {
	_K = constructIntraKernel(_X);
	initialize();
	int iter = 0;
	float tecr = 1;
...
void CML::Features::VFC::calculateGamma() {
	int numcorr = 0;
	_matchIdx.clear();
	for (int i = 0; i < _numPt; i++) {
		if (_P[i] > _theta) {
	...
	_gamma = std::min(_gamma, 0.95f);
	_gamma = std::max(_gamma, 0.05f);
}

void CML::Features::VFC::calculateSigmaSquare() {
	_sigma2 = 0;
	_sumP = 0;
	for (int i = 0; i < _numPt; i++) {
...
void CML::Features::VFC::calculateV() {
	// calculate V=K*C
	_V.clear();
	if (_method == NORMAL_VFC) {
		for (int i = 0; i < _numPt; i++) {
			...
			_V.push_back(Vector2f(t1, t2));
		}
	}
	else if (_method == FAST_VFC) {
		Matrixf<Dynamic, Dynamic> T(2, _numEig);
		for (int i = 0; i < _numEig; i++) {
			auto q = _Q.row(i);
			float t1 = 0;
			float t2 = 0;
...
void CML::Features::VFC::calculateC() {
	Matrixf<Dynamic, Dynamic> K = _K;
	for (int i = 0; i < _numPt; i++) {
		K(i, i) += _lambda*_sigma2 / _P[i];
	}
	Matrixf<Dynamic, Dynamic> Y(_numPt, 2);
	for (int i = 0; i < _numPt; i++) {
		Y(i, 0) = _Y[i].x();
		Y(i, 1) = _Y[i].y();
	}
...
void CML::Features::VFC::calculateTraceCKC() {
	// calculate K*C
	int n = _K.rows();
	List<Vector2f> KC;
	KC.clear();
...
void CML::Features::VFC::initialize() {
	_V.clear();
	_C.clear();
	_P.clear();
	for (int i = 0; i < _numPt; i++) {
		_V.push_back( Vector2f(0.0f, 0.0f) );
		_P.push_back(1.0f);
	}
	if (_method == NORMAL_VFC || _method == FAST_VFC) {
		for (int i = 0; i < _numPt; i++) {
			_C.push_back(Vector2f(0.0f, 0.0f));
		}
	}
	else if (_method == SPARSE_VFC) {
		for (int i = 0; i < _numCtrlPts; i++) {
...
CML::Matrixf<CML::Dynamic, CML::Dynamic> CML::Features::VFC::constructIntraKernel(List<Vector2f> X) {
	int n = X.size();
	Matrixf<Dynamic, Dynamic> K(n, n);
	for (int i = 0; i < n; i++) {
		K(i, i) = 1;
		for (int j = i+1; j < n; j++) {
...
CML::Matrixf<CML::Dynamic, CML::Dynamic> CML::Features::VFC::constructInterKernel(List<Vector2f> X, List<Vector2f> Y) {
	int m = X.size();
	int n = Y.size();
	Matrixf<Dynamic, Dynamic> K(m, n);
	for (int i = 0; i < m; i++) {
        auto p = K.row(i);
		for (int j = 0; j < n; j++) {
...
bool CML::Features::VFC::normalize() {
	// calculate mean
	float x1, y1, x2, y2;
	x1 = 0; y1 = 0;
	x2 = 0; y2 = 0;
	...
	for (int i = 0; i < _numPt; i++) {
		...
		Vector2f tmp(_rX[i].x() - _lX[i].x(), _rX[i].y() - _lX[i].y());
		_X.push_back(_lX[i]);
		_Y.push_back(tmp);
	}
	return 1;
