...
//...
    // logger.important("opening the tif file : " + path +"\n");
    TIFF* tif = TIFFOpen(path.c_str(), "r");

    // TIFF *tif = TIFFStreamOpen("MemTIFF", &input_TIFF_stream);

    if (!tif) {
        std::cout << "Can't open the tiff file" << std::endl;
        // return;
...
//...
    ...
    TIFF *tif = nullptr;
    tif = TIFFStreamOpen("MemTIFF", &input_TIFF_stream);

    if (!tif) {
      // logger.important("Can't open the tif file : " + std::string((char *)str) + " !\n");
        throw std::runtime_error("Can't open the tiff file");
    }
