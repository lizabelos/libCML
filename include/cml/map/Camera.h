...
namespace CML {
    ...
    public:
        ...
        EIGEN_STRONG_INLINE static Camera randomTranslation() {
            std::random_device rd;
            std::mt19937 e2(rd());
            ...
            Vector<Quaternion::N> a;
            for (int i = 0; i < 3; i++) t(i) = dist(e2);
            for (int i = 0; i < Quaternion::N; i++) a(i) = 0;

            return Camera(t, Quaternion(a));
        }

        EIGEN_STRONG_INLINE static Camera randomRotation() {
            std::random_device rd;
            std::mt19937 e2(rd());
            std::uniform_real_distribution<> dist(0, 10);
    ...
    // From ORB SLAM 2
    inline scalar_t computeDistEpipolarLine(const CML::Corner &kp1, const CML::Corner &kp2, const CML::Matrix33 &F12) {
