...
/// Generic Vocabulary
class TemplatedVocabulary : public AbstractVocabulary
{		
public:
   ...
   * @param voc
   */
  TemplatedVocabulary(const TemplatedVocabulary<TDescriptor> &voc);
   ...
   */
  TemplatedVocabulary<TDescriptor>& operator=(
    const TemplatedVocabulary<TDescriptor> &voc);
   ...
   */
  virtual void create
    (const List<List<TDescriptor> > &training_features);
   ...
   */
  virtual inline unsigned int size() const override;
   ...
   */
  virtual void transform(const List<TDescriptor>& features, BowVector &v) 
    const;
   ...
   */
  virtual void transform(const List<TDescriptor>& features,
   ...
   */
  inline double score(const BowVector &a, const BowVector &b) const override;
   ...
   */
  void getWordsFromNode(NodeId nid, List<WordId> &words) const;
...
protected:
  ...
  void getFeatures(
    const List<List<TDescriptor> > &training_features,
    List<pDescriptor> &features) const;
   ...
   */
  void HKmeansStep(NodeId parent_id, const List<pDescriptor> &descriptors,
    int current_level);
   ...
   */
  virtual void initiateClusters(const List<pDescriptor> &descriptors,
    List<TDescriptor> &clusters) const;
   ...
   */
  void initiateClustersKMpp(const List<pDescriptor> &descriptors,
    List<TDescriptor> &clusters) const;
   ...
   * @param features
   */
  void setNodeWeights(const List<List<TDescriptor> > &features);
...
template<class TDescriptor>
TemplatedVocabulary<TDescriptor>::TemplatedVocabulary
  (int k, int L, WeightingType weighting, ScoringType scoring)
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::createScoringObject()
{
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::setScoringType(ScoringType type)
{
  m_scoring = type;
  createScoringObject();
}
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::setWeightingType(WeightingType type)
{
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::create(
  const List<List<TDescriptor> > &training_features)
{
  m_nodes.clear();
  m_words.clear();
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::getFeatures(
  const List<List<TDescriptor> > &training_features,
  List<pDescriptor> &features) const
{
  features.resize(0);
  
  typename List<List<TDescriptor> >::const_iterator vvit;
  typename List<TDescriptor>::const_iterator vit;
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::HKmeansStep(NodeId parent_id, 
  const List<pDescriptor> &descriptors, int current_level)
{
  if(descriptors.empty()) return;
        
  // features associated to each cluster
  List<TDescriptor> clusters;
  List<List<unsigned int> > groups; // groups[i] = [j1, j2, ...]
  ...
  else
  {
    ...
    List<int> last_association, current_association;
    ...
    while(goon)
    {
      ...
      else
      {
        ...
        {
          List<pDescriptor> cluster_descriptors;
          ...
          */
          ...
          List<unsigned int>::const_iterator vit;
          for(vit = groups[c].begin(); vit != groups[c].end(); ++vit)
          {
            ...
            clusters[c] = TDescriptor::meanValue(cluster_descriptors);
        }
      ...
      groups.clear();
      groups.resize(clusters.size(), List<unsigned int>());
      current_association.resize(descriptors.size());

      //assoc.clear();

      typename List<pDescriptor>::const_iterator fit;
      //unsigned int d = 0;
      for(fit = descriptors.begin(); fit != descriptors.end(); ++fit)//, ++d)
      {
        double best_dist = clusters[0].distance(*(*fit));
        unsigned int icluster = 0;
        
        for(unsigned int c = 1; c < clusters.size(); ++c)
        {
          double dist = clusters[c].distance(*(*fit));
          if(dist < best_dist)
          {
            best_dist = dist;
            icluster = c;
  ...
  if(current_level < m_L)
  {
    ...
    const List<NodeId> &children_ids = m_nodes[parent_id].children;
    for(unsigned int i = 0; i < clusters.size(); ++i)
    {
      NodeId id = children_ids[i];

      List<pDescriptor> child_features;
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::initiateClusters
  (const List<pDescriptor> &descriptors,
   List<TDescriptor> &clusters) const
{
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::initiateClustersKMpp(
  const List<pDescriptor> &pfeatures,
    List<TDescriptor> &clusters) const
{
  ...
  clusters.resize(0);
  clusters.reserve(m_k);
  List<double> min_dists(pfeatures.size(), std::numeric_limits<double>::max());
  ...
  {
    *dit = clusters.back().distance(*(*fit));
  }  
  ...
  while((int)clusters.size() < m_k)
  {
    ...
    for(fit = pfeatures.begin(); fit != pfeatures.end(); ++fit, ++dit)
    {
      if(*dit > 0)
      {
        double dist = clusters.back().distance(*(*fit));
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::createWords()
{
  m_words.resize(0);
  ...
  if(!m_nodes.empty())
  {
    ...
    typename List<Node>::iterator nit;
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::setNodeWeights
  (const List<List<TDescriptor> > &training_features)
{
  ...
  else if(m_weighting == IDF || m_weighting == TF_IDF)
  {
    ...
    List<unsigned int> Ni(NWords, 0);
    List<bool> counted(NWords, false);
    
    typename List<List<TDescriptor> >::const_iterator mit;
    typename List<TDescriptor>::const_iterator fit;
...
template<class TDescriptor>
inline unsigned int TemplatedVocabulary<TDescriptor>::size() const
{
  return m_words.size();
}
...
template<class TDescriptor>
inline bool TemplatedVocabulary<TDescriptor>::empty() const
{
...
template<class TDescriptor>
float TemplatedVocabulary<TDescriptor>::getEffectiveLevels() const
{
  long sum = 0;
  typename List<Node*>::const_iterator wit;
...
template<class TDescriptor>
TDescriptor TemplatedVocabulary<TDescriptor>::getWord(WordId wid) const
{
...
template<class TDescriptor>
WordValue TemplatedVocabulary<TDescriptor>::getWordWeight(WordId wid) const
{
...
template<class TDescriptor>
WordId TemplatedVocabulary<TDescriptor>::transform
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::transform(
  const List<TDescriptor>& features, BowVector &v) const
{
  v.clear();
...
template<class TDescriptor> 
void TemplatedVocabulary<TDescriptor>::transform(
  const List<TDescriptor>& features,
  BowVector &v, FeatureVector &fv, int levelsup) const
{
  v.clear();
  fv.clear();
...
template<class TDescriptor> 
inline double TemplatedVocabulary<TDescriptor>::score
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::transform(const TDescriptor &feature, 
  WordId &word_id, WordValue &weight, NodeId *nid, int levelsup) const
{ 
  // propagate the feature down the tree
  List<NodeId> nodes;
  typename List<NodeId>::const_iterator nit;
  ...
  do
  {
    ...
    double best_d = feature.distance(m_nodes[final_id].descriptor);

    for(nit = nodes.begin() + 1; nit != nodes.end(); ++nit)
    {
      NodeId id = *nit;
      double d = feature.distance(m_nodes[id].descriptor);
      if(d < best_d)
      {
        best_d = d;
        final_id = id;
...
template<class TDescriptor>
NodeId TemplatedVocabulary<TDescriptor>::getParentNode
...
template<class TDescriptor>
void TemplatedVocabulary<TDescriptor>::getWordsFromNode
  (NodeId nid, List<WordId> &words) const
{
  words.clear();
  ...
  else
  {
    ...
    while(!parents.empty())
    {
      ...
      const List<NodeId> &child_ids = m_nodes[parentid].children;
      List<NodeId>::const_iterator cit;
...
template<class TDescriptor>
int TemplatedVocabulary<TDescriptor>::stopWords(double minWeight)
{
  int c = 0;
  typename List<Node*>::iterator wit;
  for(wit = m_words.begin(); wit != m_words.end(); ++wit)
  {
...
// --------------------------------------------------------------------------
    ...
    template<class TDescriptor>
    void TemplatedVocabulary<TDescriptor>::saveToTextFile(const std::string &filename) const
    {
        std::fstream f;
        ...
        for(size_t i=1; i<m_nodes.size();i++)
        {
            ...
            f << node.descriptor.toString() << " " << (double)node.weight << std::endl;
        }
